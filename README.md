...
| Allow premium domains within the cost limit | Off by default, and premium names are refused. With `Yes` **and** a maximum cost per year set, a premium name is allowed as long as OSIR's price for it stays within that limit — at checkout, at registration, and at renewal or transfer. Without a limit it changes nothing. See [Premium domains](#premium-domains). |
...
Registries price some names differently from the rest of their TLD — usually far above it, sometimes well
below (numeric `.xyz` names cost under a dollar). FOSSBilling sells every name of a TLD at one price, so by
default this plugin refuses premium names: you would charge your standard price and pay whatever the registry
asks.

To sell the cheap ones, turn on **Allow premium domains within the cost limit** *and* set a **maximum cost per
year**. Both are required: the limit is the only thing that makes a premium name safe, so without it premium
names stay refused.

- **At checkout**, a premium name is offered only if OSIR's price for it is at or below your limit. Anything
  dearer is not shown as available, so a customer can never pay for a name that could not then be registered.
- **At registration**, the price is checked again against the limit, as for any other name.
- **At renewal and transfer**, the same limit applies to OSIR's renewal or transfer quote. This is the case
  that matters most: a name that is cheap in its first year can renew at the premium tier, and the limit stops
  that renewal instead of letting it cost you more than you charge.
- Anything that cannot be priced with certainty is refused, as always.

Set the limit at or below what you charge for the TLD, and a premium name can never cost you more than a
standard one. A refused renewal leaves the order for an administrator: renew it at OSIR by hand, raise the
limit, or let it go.
...
|---|---|
| Domain search / checkout | Live availability check. **Premium names are refused** unless they are allowed within your cost limit (see [Premium domains](#premium-domains)): FOSSBilling has no premium pricing, so you would otherwise sell at the standard price while paying the premium price. If the check cannot be completed, the customer is asked to retry; such a check is never reported as "available" or "taken". |
...
- **Prices are checked before anything is charged.** Premium registrations are refused, and so is any price
  above your **cost limit**. Premium registration, renewal and transfer prices are refused too, unless
  [premium names are allowed within that limit](#premium-domains).
